...
use IPC::Shareable; # to handle shared memory between parallel processes
...
#	+-----------------------------------------+
	...
	dual_print ({ 'string'=> file_text_line({ string => "parse options and metadata", char => "-" }) });;

	# +-- load files --+
	dual_print ({ 'string' => "Accessing the feature_levels YAML file\n" });

	# +--locus_tag / common_tag param  --+
	dual_print ({ 'string' => "=> Attribute used to group features when no Parent/ID relationship exists (i.e common tag):\n" });
	foreach my $comTag (@{$CONFIG->{locus_tag}}	){
		dual_print ({ 'string' => "	* $comTag\n" });
	}
...
#	+-----------------------------------------+
	...
	my $nbWarnLimit = $LOGGING->{debug_mode} ? undef : 10; # limit number of warning if not debug mode
		...
		# If warning I setup in AGAT
		if ( $thematic[0] =~ /^AGAT/ ) {
			if($thematic[0] eq "AGAT GLOBAL"){ #extract global warning (about feature type in ontology and if AGAT deal with it)
		...
		# Warnings provided by perl
		else {
			dual_print ({ 'string' => $message , 'perl_warning' => 1 });
		}
	};
...
#	+-------------------------------------------------------------------------+
	...
	dual_print ({ 'string' => file_text_line({ string => "collecting general data information", char => "-" }) });
	...
	#  HERE PARALLELISATION can be done
		...
		# -------------- read GFF headers -----------------------------
		my $header = get_header_lines($file);
		$omniscient_original{'other'}{'header'}=$header if $header;
	 ...
	 # ---- Set counter for progress bar ----
		...
		# tie a new shared variable - https://metacpan.org/pod/IPC::Shareable#OPTIONS
	 ...
	 # ---- Time tracking and check processing ----
		...
		# Clean previous shared memory segment (if any) 
		#eval {
		#	my $old = tie %check_hash, 'IPC::Shareable', $check_key, {};
		#	$old->remove;
		#};
		# tie a new shared variable - https://metacpan.org/pod/IPC::Shareable#OPTIONS
		#my $check_hash_obj = tie %check_hash, 'IPC::Shareable', $check_key, {
		#	create => 1, # 1 allows creating if it doesn’t exist.
		#	exclusive => 0, # 0 allows multiple processes to access the same variable.
		#	mode => 0666, # 0666 allows read/write access to all users.
		#	destroy => 1, # If set to a true value, the shared memory segment underlying the data binding will be removed when the process that initialized the shared memory segment exits (gracefully)[1]. Only those memory segments that were created by the current process will be removed.
	 ...
	 # ---- Clean shareable if needed ----
		...
		foreach my $local_file (@files) {
			...
			$pm->start("Job $count_file") and next; {
				...
				while( my $feature = $gffio->next_feature()) {
					...
					($locusTAGvalue, $last_l1_f, $last_l2_f, $last_l3_f, $last_f, $lastL1_new) =
					manage_one_feature($ontology, $feature, \%omniscient_clean_clone, \%mRNAGeneLink, \%duplicate, \%hashID, \%locusTAG, \%infoSequential, \%attachedL2Sequential, $locusTAGvalue, $last_l1_f, $last_l2_f, $last_l3_f, $last_f, $lastL1_new);
					$nb_line_read_local++;
				}
...
sub get_attributes_as_hash_ref {
    ...
    # Loop through all tags and get their values
    foreach my $tag ($feature->get_all_tags) {
        # Store each tag's values in the hash (the tag is the key)
        $attributes{$tag} = [ $feature->get_tag_values($tag) ];
    }

    return \%attributes;  # Return a reference to the attribute hash
}
...
sub debless_and_strip_code {
    ...
    if ( blessed($ref) ){
		if ( $ref->isa('Bio::SeqFeature::Generic') ){
			# Create a hash representing the feature
			my $feature = { agat_flat_feat => {
				seq_id      => $ref->seq_id,  # Seqname (chromosome or scaffold)
				source_tag  => $ref->source_tag,  # Source
				primary_tag => $ref->primary_tag,  # Type (e.g., gene, CDS, exon)
				start       => $ref->start,  # Start position
				end         => $ref->end,  # End position
				score       => $ref->score,  # Score
				strand      => $ref->strand,  # Strand (+ or -)
				frame       => $ref->frame,  # Phase (for CDS)
				tag         => get_attributes_as_hash_ref($ref),  # Parse attributes (key-value pairs)
			} };
			return $feature ;
		}
    ...
    if (reftype($ref) eq 'HASH') {
        for my $key (keys %$ref) {
            ...
            if (defined($type) && $type eq 'CODE') {
                delete $ref->{$key};  # Remove CODE ref e.g.  sub { "DUMMY" }
            } else {
        ...
        for my $i (0 .. $#$ref) {
            ...
            my $type = reftype($val);
            if (defined($type) && $type eq 'CODE') {
                $ref->[$i] = undef;  # Remove CODE ref in arrays e.g.  sub { "DUMMY" }
            } else {
                $ref->[$i] = debless_and_strip_code($val);
            }
        }
    }
...
#		+-------------------------------+
		...
		if( keys %{$ontology} ){
			...
			if(! exists_keys($ontology, ($primary_tag) ) ) {
				warn "AGAT GLOBAL@"."ontology1@".$primary_tag."@";
			}
		}
...
#	+----------------------------------------------------------------------------+
		if( get_level($omniscient, $feature) eq 'level1' ) {
				...
				if(! _it_is_duplication($duplicate, $omniscient, $hashID, $feature, 'level1')){
						...
						# Save feature #
						$last_l1_f = $feature;
						dual_print({'string' => "::::::::::Push-L1-omniscient level1 || $primary_tag || $id = ".$feature->gff_string()."\n", 'debug_only' => 1});
						$omniscient->{"level1"}{$primary_tag}{lc($id)}=$feature;
						...
						if($locusTAGvalue){
								dual_print({'string' => "::::::::::Push-L1-sequential $locusTAGvalue || level1 == $id\n", 'debug_only' => 1});
...
# +----------------------------------------------------------------------------+
				...
				#GTF case
						...
						warn "AGAT WARNING level2: No Parent attribute found @ for the feature: ".$feature->gff_string()."\n";
						...
						if(! $last_l1_f or ($locusTAGvalue and (lc($locusTAGvalue) ne lc($last_locusTAGvalue) ) ) ){
								dual_print({'string' => "create L1 feature\n", 'debug_only' => 1});
								...
								if ( ( ( $lastL1_new and not exists_keys( $LEVELS, ('level2', $last_f->primary_tag ) ) )
									and	(!$locusTAGvalue or (lc($locusTAGvalue) ne lc($last_locusTAGvalue) ) ) ) # if previous L1 newly created and last feature is not f2 (if several f2 in a row we attach them to the same newly created l1 feature)
										...
										dual_print({'string' => "create L1 feature stritcly\n", 'debug_only' => 1});
										if($locusTAGvalue){ # Use the locus tag instead to create a newID
											$l1_ID = $locusTAGvalue;
										}
										else{
										...
										dual_print({'string' => "take last L1 feature\n", 'debug_only' => 1});
						...
						if($locusTAGvalue){ #Previous Level up feature had a comon tag
								dual_print({'string' => "::::::::::Push-L2-Sequential-1 $locusTAGvalue || ".$id." || level2 == ".$feature->gff_string."\n", 'debug_only' => 1});
								...
								dual_print({'string' => "::::::::::Push-L2-omniscient-2: level2 || ".$primary_tag." || ".$l1_ID." == ".$feature->gff_string."\n", 'debug_only' => 1});
				...
				if(! _it_is_duplication($duplicate, $omniscient, $hashID, $feature, 'level2')){
						...
						if (! exists ($mRNAGeneLink->{lc($id)})){
				 		...
				 		####################
				 		# SAVE THE FEATURE #
				 		dual_print({'string' => "::::::::::Push-L2-omniscient-3 level2 || $primary_tag || $parent == ".$feature->gff_string."\n", 'debug_only' => 1});
...
# +----------------------------------------------------------------------------+
				...
				# No parent case	# But the feature itself stay intact without parentID.
						...
						my $play_this_game=1;
						warn "AGAT WARNING level3: No Parent attribute found @ for the feature: ".$feature->gff_string()."\n";
						...
						if($last_l2_f and $locusTAGvalue){
								...
								if(exists_keys ($locusTAG_uniq, ('linkl2l1', lc($last_l2_f->_tag_value('ID') ) ) ) ){
										if (lc($locusTAG_uniq->{'linkl2l1'}{lc( $last_l2_f->_tag_value('ID') )}) ne lc($locusTAGvalue)){
												$skip_last_l2=1;
												dual_print({'string' => "skip last l2\n", 'debug_only' => 1});
										}
								} else {
										$skip_last_l2=1;
										dual_print({'string' => "skip last l2\n", 'debug_only' => 1});
						...
						if(! $last_l2_f or
						  ($locusTAGvalue and ( lc($locusTAGvalue) ne lc($last_locusTAGvalue) ) and ( lc($last_locusTAGvalue) ne lc($parent_of_last_l2) or $skip_last_l2) ) ){
								dual_print({'string' => "Come in the complex case L3!!!\n", 'debug_only' => 1});
								#######################
								...
								if( $locusTAGvalue and exists_keys($locusTAG_uniq, ('level2', lc($locusTAGvalue) ) ) ){
										dual_print({'string' => "Complex case L3 1 !!!\n", 'debug_only' => 1});
										...
										dual_print({'string' => "Complex case L3 2 !!!\n", 'debug_only' => 1});
										...
										if($last_l3_f and $last_l3_f->has_tag('Parent')){ # Need to not be the first feature because we need a previous feature
												...
												dual_print({'string' => "Complex case 2.1 !!!\n", 'debug_only' => 1});
												...
												if ($locusTAGvalue and $previousL3 and (lc($locusTAGvalue) eq lc($previousL3) ) ){
														...
														dual_print({'string' => "Complex case 2.1.1 !!!\n", 'debug_only' => 1});
						...
						# case where previous level2 exists
							...
							dual_print({'string' => "case where previous level2 exists\n", 'debug_only' => 1});
							$l2_id = $last_l2_f->_tag_value('ID');
						}
						...
						if ($play_this_game){
								...
								if($locusTAGvalue){ #Previous Level up feature had a comon tag
										...
										if(!$last_l1_f and $last_l3_f){ #particular case : Two l3 that follow each other, but first one has locus_tag but not the second
												dual_print({'string' => "::::::::::Push-L3-sequential-2 $last_locusTAGvalue || ".lc($l2_id)." || level3 == ".$feature->gff_string."\n", 'debug_only' => 1});
												...
												$infoSequential->{'id'}{lc($l2_id)} = $l2_id;
												dual_print({'string' => "::::::::::Push-L3-omiscient-3: level3 ".$primary_tag." || ".lc($l2_id)." == ".$feature->gff_string."\n", 'debug_only' => 1});
				...
				if(! exists_keys($omniscient,('level3',$primary_tag,lc($parent)))){
					...
					if($allParent > 1){
						...
						# It is the first parent. Do not clone the feature
							...
							dual_print({'string' => "::::::::::Push-L3-omniscient-5 level3 || $primary_tag || ".lc($parent)." == ".$feature->gff_string."\n", 'debug_only' => 1});
							push (@{$omniscient->{"level3"}{$primary_tag}{lc($parent)}}, $feature);
						}
					}
					else{ #the simpliest case. One parent only
						dual_print({'string' => "::::::::::Push-L3-omniscient-6 level3 || $primary_tag || ".lc($parent)." == ".$feature->gff_string."\n", 'debug_only' => 1});
						push (@{$omniscient->{"level3"}{$primary_tag}{lc($parent)}}, $feature);
					}
				}
...
# +----------------------------------------------------------------------------+
				...
				warn "AGAT GLOBAL@"."parser1@".$primary_tag."@";
	...
	foreach my $tag_l1 ( sort {$a cmp $b} keys %{$omniscient->{'level1'}}){ # primary_tag_key_level2 = mrna or mirna or ncrna or trna etc...
		if ( exists_keys ( $omniscient, ('level1', $tag_l1, lc($parent) ) ) ){
			...
			if( $feature->seq_id() ne $omniscient->{'level1'}{$tag_l1}{lc($parent)}->seq_id()  )	{
				warn "AGAT WARNING l2 and l1 features not on same seq_id @ ".$feature->_tag_value("ID").
...
sub _get_comon_tag_value{
	...
	if(! $locusName_lc and $level ne 'level1'){
		warn "AGAT WARNING gff3 reader: Hmmm, be aware that your feature doesn't contain any Parent and locus tag.".
...
sub _it_is_duplication{
	...
	if(! $is_dupli and $level eq "level1" and $omniscient->{"level1"}{$primary_tag}{$id}){
		warn "AGAT WARNING level1: This feature level1 is not a duplicate but has an ID already used.\n".
...
sub get_level{
	...
	if ( exists_keys($LEVELS, ('level1', lc($feature->primary_tag) ) ) ){
		return 'level1';
	}
	elsif( exists_keys($LEVELS, ('level2', lc($feature->primary_tag) ) ) ){
		return 'level2';
	}
	elsif( exists_keys($LEVELS, ('level3', lc($feature->primary_tag) ) ) ){
		return 'level3';
	}
}
...
sub _check_l1_linked_to_l2{
		...
		foreach my $id_l1 ( sort {$a cmp $b} keys %{$hash_omniscient->{'level2'}{$primary_tag_l2}}){ # primary_tag_key_level2 = mrna or mirna or ncrna or trna etc...
						...
						if ( exists_keys( $hash_omniscient, ('level3', $primary_tag_l3, lc($l2_id) ) ) ){
							...
							foreach my $ParentIDinfo (@ParentIDsinfo){
								...
								if (lc($ParentID) ne lc ($id_l1) ){
									...
									dual_print({ 'string' => "L2 subfeature shows a different L1 feature relationship. It shows $ParentID instead of $id_l1!\n" });
							...
							if ($change_parent){
								dual_print({ 'string' => "$l2_id: Parent ID $id_l1 was wrong, we found a common tag from l3 feature indicating to use $change_parent instead!\n" });
								...
								if(!$cloned_l2_feature){
									dual_print({ 'string' => "$l2_id not found for feature l2 stored in omniscient level2,$primary_tag_l2,$id_l1\n" });
								}
								else{
			...
			if(! $l1_exist){
				...
				if ( lc( $new_ID_l1 ) ne  lc($oID) ){
					dual_print({'string' => "Parent ID and ID are the same. Here is the new parent ID created $new_ID_l1.\n", 'debug_only' => 1});
				...
				dual_print({'string' =>  "No Parent feature found for ".$l2_id.". We create one: ".$gene_feature->gff_string()."\n", 'log_only' => 1}); # print only in log
			}
		}
	}
	...
	if($resume_case){
		dual_print({ 'string' => "$resume_case cases fixed where L2 features have parent features missing\n" });
	}
	else{
		dual_print({ 'string' => "No problem found\n" });
	}
}
...
sub _remove_orphan_l1{
	my ($omniscient, $hashID, $mRNAGeneLink)=@_;
	my $resume_case=undef;

 	foreach my $tag_l1 (keys %{$omniscient->{'level1'}}){
 			foreach my $id_l1 (keys %{$omniscient->{'level1'}{$tag_l1}} ){
				if ( exists_keys( $LEVELS, ( 'level1', $tag_l1) ) ){
					if ($LEVELS->{'level1'}{$tag_l1} eq 'standalone' or
								$LEVELS->{'level1'}{$tag_l1} eq 'topfeature'){

						dual_print({'string' => "skip $tag_l1 because is suppose to be orphan\n", 'debug_only' => 1}); # print only in log
						next;
					}
				}
				...
				my $neverfound="yes";
 				...
 				if($neverfound){
 					$resume_case++;
					dual_print({'string' => "removing ".$omniscient->{'level1'}{$tag_l1}{$id_l1}->gff_string."\n", 'log_only' => 1}); #print only in log
	...
	if($resume_case){
		dual_print({ 'string' => "$resume_case cases removed where L1 features do not have children (while they are suposed to have children).\n" });
	}
	else{
		dual_print({ 'string' => "None found\n" });
	}
}
...
sub _check_l2_linked_to_l3{
	my ($hash_omniscient, $mRNAGeneLink, $hashID)=@_;
	my $resume_case=undef;
	...
	if($resume_case){
 		dual_print({ 'string' => "$resume_case cases fixed where L3 features have parent feature(s) missing\n" });
	}
	else{
		dual_print({'string' => "No problem found\n" });
	}
...
sub _check_all_level3_locations{
	...
	foreach my $type_l3 (keys %{$hash_omniscient->{'level3'}}){
		foreach my $id_l2 (keys %{$hash_omniscient->{'level3'}{$type_l3}}){
			if(! exists_keys($LEVELS,( 'skip_merge_l3', $type_l3) ) ){
					if( exists_keys($hash_omniscient,('level3', $type_l3, $id_l2) ) ){
						...
						my $method;
						exists_keys($LEVELS,('spread', $type_l3) ) ? $method = "adjacent" : $method = "all";
						$nb_merged = merge_features( $hash_omniscient, 'level3', $type_l3, $id_l2, $method);
					}
			}
		}
		if($nb_merged){
	...
	foreach my $type (keys %resume_cases){
		dual_print({ 'string' => "$resume_cases{$type} adjacent $type merged\n" });
	}
}
...
sub _check_cds{
 			...
 			foreach my $id_l2 ( sort {$a cmp $b} keys %{$hash_omniscient->{'level3'}{'cds'}} ) {
				if( exists_keys($hash_omniscient,('level3', 'stop_codon',$id_l2)) ){
					...
					if($#list_stop > 0){
							...
							if($size_stop > 3){
								dual_print({ 'string' => "Warning: $id_l2 has several stop_codon\n" });
								next;
							}
							else{
								$codon_split = 1;
								dual_print({ 'string' => "Stop codon split over exons\n", 'log_only' => 1 });
							}
					}
						...
						if($cds->end +1 != $stop->start){ # Last stop in not adjacent to CDS.
							...
							if($codon_split){ # Not adjacent because stop splited
								...
								if ($cds->end + 1 == ($list_stop[0]->start) ){
									dual_print({ 'string' => "Extend CDS to the first part of the stop codon\n", 'log_only' => 1 });
								...
								if (! $stop_start_exon){
									dual_print({ 'string' => "Warning: $id_l2 stop codon not adjacent to the CDS\n" });
								}
							}
						}
						# stop is adjacent
	...
	if( $resume_case ){
		dual_print({ 'string' => "$resume_case CDS extended to include the stop_codon\n" });
	}
	if($resume_case2){
		dual_print({ 'string' => "$resume_case2 CDS created to include the stop_codon that was on next exon\n" });
	}
...
#				 	+-----------------------------------------------------
						...
						foreach my $tag_l3 ( sort {$a cmp $b} keys %{$hash_omniscient->{'level3'}}){
							...
							if ($tag_l3 ne "exon" and $LEVELS->{'level3'}{$tag_l3} eq "exon" ){
							...
							if( exists_keys($hash_omniscient,('level3',$tag_l3, $id_l2)) ){
									...
									foreach my $l3_feature (@{$hash_omniscient->{'level3'}{$tag_l3}{$id_l2}}){
				 					...
				 					if(@{$list_location_Exon} < @{$hash_omniscient->{'level3'}{$tag_l3}{$id_l2}}){
				 						dual_print({'string' => "Peculiar rare case, we have to remove existing exon which are supernumerary. Parent is $id_l2\n", 'log_only' => 1 }); #print log only
									...
									foreach my $locations (@{$list_location_Exon}){
				 					...
				 					dual_print({'string' => "We remove the supernumerary @id_list2 exon(s)\n", 'log_only' => 1}); #print log only
...
#				 	+--------------------------------------------------------------------------------------------------------+
	 					...
	 					if( exists_keys($hash_omniscient,('level3','exon', $id_l2)) ){ #When thre are l3 features but no exon among them... we need to recreate them.
							...
							if($list_location_Exon_joined ne $list_location_NoExon_joined ){
								dual_print({'string' => "Problem for $id_l2! coordinates of exons found not in\n".
											...
											if ($redefine_left or $redefine_right){
												foreach my $l3_feature (@{$hash_omniscient->{'level3'}{'exon'}{$id_l2} } ){
													if($l3_feature->_tag_value('ID') eq $exon_location->[0][0]){
														...
														if($redefine_left){
															dual_print({'string' => "Modify the left location for ".$l3_feature->_tag_value('ID')." from ".$l3_feature->start()." to ".$new_location->[1]."\n", 'log_only' => 1}); #print log only
															$l3_feature->start($new_location->[1]); $resume_case2++;
														}

														if($redefine_right){
															dual_print({'string' => "Modify the right location for ".$l3_feature->_tag_value('ID')." from ".$l3_feature->end()." to ".$new_location->[2]."\n", 'log_only' => 1}); #print log only
							...
							dual_print({ 'string' => "No other feature to check the exon locations (e.g CDS, UTR, etc...). We can trust them then.\n", 'debug_only' => 1});
						}
	 				}
	 								...
	 								if( lc($l2_feature->_tag_value('ID')) eq $id_l2 ){
	 								 	if( exists_keys ($hash_omniscient, ('level3', 'exon', $id_l2))) { # If no exon it could be a case whre no L3 feature need an exon like non_canonical_three_prime_splice_site (they are out of exon). So the list of exon does not exist.
					 					 	...
					 					 	if( int($list_exon[0]->start) >	int($myLeftExtremity) ){
												dual_print({ 'string' => "Modify the exon ".$list_exon[0]->_tag_value('ID')." LEFT extremity for $id_l2! From ".$list_exon[0]->start." to ".$myLeftExtremity."\n", 'log_only' => 1});
					 					 		$list_exon[0]->start($myLeftExtremity);
												$resume_case2++;
					 					 	}
					 					 	if($list_exon[0]->start <	$myLeftExtremity){	#modify L2
												dual_print({ 'string' => "Modify the L2 $id_l2 LEFT extremity! From ".$l2_feature->start."to".$list_exon[0]->start."\n", 'log_only' => 1});
					 					 		$l2_feature->start($list_exon[0]->start);
												$resume_case4++;
					 					 	}

					 					 	if($list_exon[$#list_exon]->end <	$myRightExtremity){
					 					 		dual_print({ 'string' => "Modify the exon ".$list_exon[$#list_exon]->_tag_value('ID')." RIGHT extremity for $id_l2! From ".$list_exon[$#list_exon]->end." to ".$myRightExtremity."\n", 'log_only' => 1});
					 					 		$list_exon[$#list_exon]->end($myRightExtremity);
												$resume_case2++;
					 						}
					 						elsif($list_exon[$#list_exon]->end >	$myRightExtremity){ #modify L2
												dual_print({ 'string' => "Modify the L2 $id_l2 RIGHT extremity! From ".$l2_feature->end."to".$list_exon[$#list_exon]->end."\n", 'log_only' => 1});
	...
	if( $resume_case ){
		dual_print({'string' => "$resume_case exons created that were missing\n"});
	}
	else{ 
		dual_print({'string' => "No exons created\n"});
	}
	if( $resume_case2 ){
		dual_print({'string' => "$resume_case2 exons locations modified that were wrong\n"});
	}
	else{ 
		dual_print({'string' => "No exons locations modified\n"});
	}
	if( $resume_case3 ){
		dual_print({'string' => "$resume_case3 exons removed that were supernumerary\n"});
	}
	else{ 
		dual_print({'string' => "No supernumerary exons removed\n"});
	}
	if( $resume_case4 ){
		dual_print({'string' => "$resume_case4 level2 locations modified\n"});
	}
	else{ 
		dual_print({'string' => "No level2 locations modified\n"});
	}
}
...
#				 	+-----------------------------------------------------
					if( exists_keys($hash_omniscient,('level3','cds', $id_l2)) ){ #Check UTR only if CDS exists
						...
						foreach my $exon_location (sort {$a->[1] <=> $b->[1] } @{$list_location_Exon}){
							...
							foreach my $location_cds (sort {$a->[1] <=> $b->[1] } @{$list_location_CDS}){
								...
								($new_location, $overlap) = _manage_location_lowLevel_inversed($location_cds, $exon_location);
								...
								if($new_location->[1] != $exon_location->[1] and $new_location->[2] != $exon_location->[2] ){ #two UTR expected				=========================	exon
									dual_print({ 'string' => "creation utr push1\n", 'debug_only' => 1 });
							...
							if($never_overlap){ #case of UTR that match++ fully the exon
	 						...
	 						if($list_location_UTR){ #List UTR not empty
								if($list_location_UTR_expected){ #List UTR not empty
				 					foreach my $UTRexp_location (sort {$a->[1] <=> $b->[1] } @{$list_location_UTR_expected} ){
			 							...
			 							foreach my $UTR_location (sort {$a->[1] <=> $b->[1] } @{$list_location_UTR}){
			 								...
			 								($new_location, $overlap) = _manage_location_lowLevel_inversed($UTR_location, $UTRexp_location); #just to check that it overlaps
				 										...
				 										if( exists_keys($hash_omniscient,('level3', $tag_l3, $id_l2)) ){
							 								foreach my $l3_feature (@{$hash_omniscient->{'level3'}{$tag_l3}{$id_l2} } ){
							 									if($l3_feature->_tag_value('ID') eq $UTR_location->[0][0] and $l3_feature->start eq $UTR_location->[1] and $l3_feature->end eq $UTR_location->[2]){ # we have to check position to be sure we modify the correct one, because UTR could share the same ID
																	...
																	$resume_case2++;
																	dual_print({ 'string' => "Modify the UTR: ".$UTR_location->[0][0]." location for $id_l2! From ".$UTR_location->[1]." ".$UTR_location->[2]." to ".$UTRexp_location->[1]." ".$UTRexp_location->[2]."\n", 'log_only' => 1 }); # print log only
							 										last;
							 									}
							 								}
							 							}
								...
								dual_print({ 'string' => "Error in the file, we have an UTR but none".
								...
								dual_print({ 'string' => "Remove $nb_supernumary_utrs supernumerary UTRs for $id_l2\n", 'log_only' => 1 });
							}
		 				}
	 					else{
	 						if($list_location_UTR_expected){
						...
						if($list_utr_to_create){
								# sort by start position
					 			foreach my $location (sort {$a->[1] <=> $b->[1]} @{$list_utr_to_create}){
									...
									#save new feature L2
									dual_print({ 'string' => "Create one UTR for $id_l2\n:".$feature_utr->gff_string."\n", 'log_only' => 1 });
	...
	if( $resume_case ){
		dual_print({ 'string' => "$resume_case UTRs created that were missing\n" });
	}
	else{ 
		dual_print({ 'string' => "No UTRs created\n"}); 
	}
	if( $resume_case2 ){
		dual_print({ 'string' => "$resume_case2 UTRs locations modified that were wrong\n"});
	}
	else{ 
		dual_print({ 'string' => "No UTRs locations modified\n"}); 
	}
	if( $resume_case3 ){
		dual_print({ 'string' => "$resume_case3 UTRs removed that were supernumerary\n"});
	}
	else{ 
		dual_print({ 'string' => "No supernumerary UTRs removed\n"});
	}
}
...
sub merge_features{
	my ($hash_omniscient, $level, $tag_l3, $id_l2, $method) = @_;
...
sub _deinterleave_sequential{
	my ($infoSequential, $locusTAG_uniq) = @_;
	my $resume_case=undef;
	...
	foreach my $locusNameHIS (keys %{$infoSequential->{'locus'}} ){
	 	...
	 	if(exists_keys($locusTAG_uniq,('level1', $locusNameHIS))){
	 		...
	 		if($locusNameHIS ne $locusNameUniq ){
				...
				dual_print({ 'string' => "Locus $locusNameHIS interleaved\n", 'log_only' => 1}); # print only in log
				$resume_case++;
	...
	if($resume_case){
		dual_print({ 'string' => "$check_cpt - Number of interleaved locus ( Features of the locus data are defined earlier in the file): $resume_case\n" });
	}
}
...
sub _check_sequential{ # Goes through from L3 to l1
 	my ($infoSequential, $omniscient, $hashID, $locusTAG_uniq, $mRNAGeneLink) = @_;
 	my $resume_case_l2=0;
 	my $resume_case_l3=0;

 	_deinterleave_sequential($infoSequential, $locusTAG_uniq); # PART OF LOCUS LOST BEFORE TO MEET ITS L2 or L1 ... we catch them and re-link everything as it should be
		...
		foreach my $bucket (sort { ncmp($a,$b) } keys %{$infoSequential->{'locus'}{$locusNameHIS} } ){ #bucket = level1 or Id L2
			...
			# NO LEVEL3 FEATURE FOR THIS BUCKET/LOCUS
 			if(! exists_keys($infoSequential,("locus", $locusNameHIS, $bucket, 'level3') ) ){
					...
					dual_print({ 'string' => "level2 in sequential does not have L3 feature associated in sequential".
					...
					if( ! exists_keys($omniscient,('level3', "exon", lc($feature_l2->_tag_value("ID")))) ){ #check if an exon exist in the omniscient
	 				...
	 				#warn "AGAT Not normal, we have feature L2	without L3 feature associated.\n";
 				...
 				foreach my $feature_L3 (@{$infoSequential->{'locus'}{$locusNameHIS}{$bucket}{'level3'}} ){
					...
					if(! $feature_l2){
	 					if(! exists_keys($infoSequential,("locus", $locusNameHIS, $bucket,'level2'))	){
		 						dual_print({ 'string' => "_check_sequential level2 does not exits in sequential !\n", 'debug_only' => 1 }); 
		 								...
		 								if(exists_keys($omniscient, ('level2', $tag_l2, lc($l1_id) ) ) ){
				 							foreach my $featureL2 (@{$omniscient->{'level2'}{$tag_l2}{lc($l1_id)}}){
				 								if(lc($featureL2->_tag_value('ID')) eq $bucket ){
				 									dual_print({ 'string' => "_check_sequential level2 exits in omniscient !\n", 'debug_only' => 1 }); 
								...
								#If locus_tag check from omniscient if feature has same locus tag
										...
										dual_print({ 'string' => "FeatureA has the common tag value shared with a featureX from omniscient.".
									...
									dual_print({ 'string' => "_check_sequential ID created for level2: $bucket\n", 'debug_only' => 1});
									...
									my $parentID = undef;
								 	if( exists_keys($infoSequential,("locus", $locusNameHIS,'level1'))	){ # parent ID exists in infoSequential
		 								...
		 								dual_print({ 'string' => "_check_sequential Parent ID for level2 taken from infoSequential: $parentID\n", 'debug_only' => 1});
		 							}
									else{
										my $IDgoodCast = _id_exists_in_l1_omniscient($omniscient, $locusNameHIS);
										if($IDgoodCast){
												$parentID = $IDgoodCast;
												dual_print({ 'string' => "_check_sequential Parent ID for level2 taken from omniscient: $parentID\n",'debug_only' => 1});
										}
							...
							if(! exists($mRNAGeneLink->{$bucket}) ){
								push (@{$omniscient->{"level2"}{lc($feature_l2->primary_tag)}{lc($feature_l2->_tag_value('Parent'))} }, $feature_l2);
								$mRNAGeneLink->{lc($feature_l2->_tag_value('ID'))} = $feature_l2->_tag_value('Parent');
								dual_print({ 'string' => "level2 does not exits in mRNAGeneLink(omniscient) !".$feature_l2->gff_string."\n", 'debug_only' => 1 });
								$resume_case_l2++;
							}
						}
					}
					...
					dual_print({ 'string' => "push-omniscient: level3 || ".$primary_tag_L3." || ".$bucket." == ".$feature_L3->gff_string."\n", 'debug_only' => 1 });
					$resume_case_l3++;
				}
			}
	...
	if($resume_case_l2){
		dual_print({ 'string' => "Check$check_cpt - Number level2 sequential cases: $resume_case_l2\n"});
	}
	elsif ($resume_case_l3){
		dual_print({ 'string' => "Check$check_cpt - Number level3 sequential cases: $resume_case_l3\n"});
	}
	else{
		dual_print({ 'string' => "Check$check_cpt - $NOTHING_MESSAGE\n"});
	}
...
sub _check_identical_isoforms{
	...
	if($resume_case){
		dual_print({ 'string' => "Check$check_cpt - identical isoforms removed: $resume_case\n"});
	}
	else{
		dual_print({ 'string' => "Check$check_cpt - $NOTHING_MESSAGE\n"});
	 }
...
sub _check_duplicates{
	my ($duplicate, $omniscient) = @_	;

		my $keyExist = keys %{$duplicate};
		if($keyExist){#print result
			dual_print({ 'string' => "Duplicates detected! (Same chr/contig/scaffold, same position, same ID)\n"});
...
sub get_general_info{
	...
	dual_print({ 'string' => "=> Fasta included: $fasta_included\n"});
	dual_print({ 'string' => "=> Number of features lines: $nb_feature_line\n"});

	# ----- inform problem of line wihtout the 9 fields expected ----
	foreach my $size (keys %nb_field){
		if($size != 9){
			my $nb_field_error = $nb_field{$size};
			dual_print({ 'string' => "=> Number of feature lines with $size fields (while 9 expected): $nb_field_error\n"});
		}
	}
	...
	foreach my $ft (keys %feature_type){
		...
		} elsif  ( exists_keys($LEVELS, ('level2', lc($ft) ) ) ){
			push (@{$info_levels{"level2"}}, $ft);
		} elsif ( exists_keys($LEVELS, ('level3', lc($ft) ) ) ){
	...
	if(@listL3 and !(@listL1 and @listL2)){
		...
		dual_print({ 'string' => " * Number of feature with Parent attribute:$nb_parent"});
		dual_print({ 'string' => " * Number of feature with a common attribute:$nb_common_tag\n"});
		# Nothing missing
		if ($nb_parent >= $nb_feature_line and $nb_common_tag >= $nb_feature_line){
			dual_print({ 'string' => "  => Everything should be fine, we can even reconstruct isoforms if any!\n"});
		}
		# Missing Parent and common attribute
		elsif ($nb_parent == 0 and $nb_common_tag == 0){
			dual_print({ 'string' => "  => Common attributes and Parent attributes missing.\n".
			...
			dual_print({ 'string' => "  => Some common attributes and some Parent attributes missing.\n".
		...
		# Missing Parent attribute
			...
			dual_print({ 'string' => "  => Parent attributes missing but common attributes present.\n".
		...
		# Missing common attribute
			...
			if ($nb_common_tag == 0 or $nb_common_tag == 1){
				dual_print({ 'string' => "  => Common attributes missing.\n".
				  "  /!\\ In Eukaryote, if you expect isoforms in your annotation: ".
			...
			# Some common attribute are missing
				...
				dual_print({ 'string' => "  => Some common attributes missing.\n".
				  "  /!\\ In Eukaryote, if you expect isoforms in your annotation: ".
			...
			dual_print({ 'string' => "  !! You might try to fix the issue by choosing a common attribute to use in order to group the features correctly ".
					"(parameter locus_tag in agat config file agat_config.yaml (to access it type: agat config --expose)).\n"});
		}
	}
...
sub select_gff_format{
	...
	if (%gff_in_format){
		...
		if ($nb_col == 8){
			...
			dual_print({ 'string' => surround_text("Your file has less than 8 columns ($nb_col). It cannot be a GTF/GFF file. Please verify your file",80,"!") });
			exit;
		}
		else{
	...
	if ($nb_col_in_attribute > 1){
		dual_print({ 'string' => surround_text("Interesting this GTF/GFF file has tabulation(s) within the attributes, this is not supposed to happen. FYI tabs must be replaced with the %09 URL escape in GFF3 or C (UNIX) style backslash-escaped representation \\t in GFF2.",80,"!") });
		$gff_in_format{1}++;
	}
...
sub create_term_and_id_hash{
		...
		dual_print({ 'string' => "	Filtering ontology:\n" });
		dual_print({ 'string' =>  "The feature type (3rd column) is constrained to be either a term from the Sequence ".
		...
		"(SO:0000110) or an is_a child of it.\nWe filter the ontology to apply this rule.", 'log_only' => 1}); # print in log only
		...
		dual_print({ 'string' => "		We found ".(scalar @descendants)." terms that are sequence_feature or descendant terms of it (is_a constraint not applied).\n", 'debug_only' => 1});
		# get a is_a relationship
		my $is_a = Bio::Ontology::RelationshipType->get_instance('IS_A');
		@descendants = $ontology->get_descendant_terms($term, $is_a);
		dual_print({ 'string' => "		We found ".(scalar @descendants)." terms that are sequence_feature or is_a child of it.\n"});
...
sub fetcher_JD {
	my ($url, $target) = @_;
		...
		if ($response->is_success) {
			if($target){
			 	open my $OUT, '>', $target or dual_print ({ 'string' =>  "File error: $! :: $?", 'local_verbose' => 1 }) && die;
				...
				dual_print({ 'string' => $response->status_line, 'local_verbose' => 1 }) && die;
		}
}
...
sub _handle_ontology{
	...
	dual_print({ 'string' => "=> Accessing Ontology\n"});
	...
	if(exists_keys($gff3headerInfo, ("##feature-ontology"))){
		...
		dual_print({ 'string' => "	feature-ontology URI defined within the file: ".$sofa_file_path."\n"});
		#retrieve the data from URI and save it in a string
		my $stringFILE=undef;
		try{
			$stringFILE = fetcher_JD($sofa_file_path, undef);
		}
		catch{
			dual_print({ 'string' => "	The URI provided (".$sofa_file_path.") doesn't work.\n" });
			dual_print({ 'string' => "error: $_\n" });
		};
		...
		if($stringFILE){
			...
			open( my $fh_uriOnto, '<', $tmp_onology_file) or
							dual_print({ 'string' => "Cannot read the string: $! :: $?", 'local_verbose' => 1}) && die;
			...
			#parse the ontology saved
	 		...
	 		try{
	 			...
	 			dual_print({ 'string' => "	The URI provided doesn't point to obo ontology format data.\n"});
	 			dual_print({ 'string' => "error: $_\n"});
	 			$parser = undef;
	 		};
			...
			if($parser){ #We got ontology at the URI location, no need to use the internal one
				$internalO=undef;
				dual_print({ 'string' => "	feature-ontology parsed correctly\n"});
			}

			# remove tmp file
			unlink $tmp_onology_file or
							dual_print({ 'string' => "Cannot remove the temporary file $tmp_onology_file: $! :: $?", 'local_verbose' => 1 }) && die;
		}
	}
	...
	if($internalO){ #No URI provided for the feature-ontology(file case), or doesn't exist (hash / table case) let's use the interal one
		dual_print({ 'string' => "	No ontology accessible from the gff file header!\n"});
		try{
			$sofa_file_path = dist_file('AGAT', 'so.obo');
			dual_print({ 'string' => "	We use the SOFA ontology distributed with AGAT:\n		$sofa_file_path\n"});
			...
			dual_print({ 'string' => "error: $_\n"});
 			dual_print({ 'string' => "	Let's continue without feature-ontology information.\n"});
		};
	}
	...
	if($ontology_obj) {
		...
		dual_print({ 'string' => "	Read ontology $sofa_file_path:\n".
...
sub _handle_globalWARNS{
	...
	if( lc($type) eq "ontology" ){
		dual_print({ 'string' => file_text_line({ string => "ontology", char => "-" }) });
		my $string;
		if(exists_keys( $globalWARNS, ("ontology1") ) ) {
			if( keys %{$ontology} ){
				my %hash	 = map { $_, 1 } @{$globalWARNS->{ontology1}};
				my @unique = sort keys %hash;
				$string = "Check$check_cpt - INFO - Feature types not expected by the GFF3 specification:\n* ".
				join("\n* ", @unique).
				...
				$string = "Check$check_cpt - No ontology was available, we haven't checked if the feature types (3rd column) correspond to the gff3 specifactions.";
			}
		}
		else{
			$string = "Check$check_cpt - All feature types in agreement with the Ontology.";
		}
		dual_print({ 'string' => print_wrap_text($string,80) });
	}
	if( lc($type) eq "agat" ){
		dual_print({ 'string' => file_text_line({ string => "agat", char => "-" }) });
		my $string;
		if(exists_keys($globalWARNS, ("parser1") ) ) {
			my %hash	 = map { $_, 1 } @{$globalWARNS->{parser1}};
			my @unique = sort keys %hash;
			$string = "Check$check_cpt - WARNING - Feature types not expected by AGAT:\n* ".
			join("\n* ", @unique).
			...
			$string = "Check$check_cpt - AGAT can deal with all the encountered feature types (3rd column)";
		}
		dual_print({ 'string' => print_wrap_text($string,80) });
	}
}
...
sub _create_log_file{
	...
	# create a log file
	open($log, '>', "$AGAT_LOG/$filename"  ) or
				warn "Can not open $AGAT_LOG/$filename for printing log: $!" && die;
