...
for i in {0..50}_*;do
	...
	if [[ -f $i ]];then
		...
		if [[ ! $i =~ ^[[:digit:]]+_correct ]];then
			...
			nb=${i%"_test.gff"}
			if (( $nb == 28 ));then
				agat_sp_gxf_to_gff3.pl --gff $i -o test.gff3 -c Name  &> /dev/null
			#case with prokaryote mode needed
			elif (( $nb == 8  ));then
				agat_sp_gxf_to_gff3.pl --gff $i -o test.gff3   &> /dev/null
			#others
			else
				agat_sp_gxf_to_gff3.pl --gff $i -o test.gff3 --merge_loci &> /dev/null
			fi
			...
			nb=${i%"_test.gff"}
			if (( $nb == 8 || $nb == 28 ));then
				agat_sp_gxf_to_gff3.pl --gff test.gff3 -o test2.gff3  &> /dev/null
			else
				agat_sp_gxf_to_gff3.pl --gff test.gff3 -o test2.gff3  --merge_loci &> /dev/null
			fi
