...
my $script = $script_prefix . catfile($bin_dir, "agat_convert_sp_gff2bed.pl");

{ ok(system("$script -h 1>\/dev\/null") == 0, "help $script"); }

{
    my $dir = setup_tempdir();
    my $outtmp = catfile($dir,'tmp.bed');
    my $result = "$output_folder/agat_convert_sp_gff2bed_1.gff";
    check_quiet_run(
        "$script --gff " . catfile($input_folder,'1.gff') . " -o $outtmp"
    );
    check_diff($outtmp, $result, 'output agat_convert_sp_gff2bed.pl');
}
