...
my $script = $script_prefix . catfile($bin_dir, "agat_sp_extract_attributes.pl");
...

{
    ...
    my $outtmpres = catfile($dir,'tmp_protein_id.gff');
    check_quiet_run(
        "$script --gff " . catfile($input_folder,'1.gff') .
        " --att protein_id -o $outtmp"
    );
    check_diff($outtmpres, $result, 'output agat_sp_extract_attributes.pl');
}
