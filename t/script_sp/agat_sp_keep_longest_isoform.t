...
my $script = $script_prefix . catfile($bin_dir, "agat_sp_keep_longest_isoform.pl");
...

{
    ...
    check_diff($outtmp, $result, 'output keep_longest_isoform case1');
}

{
    my $dir = setup_tempdir();
    my $outtmp = catfile($dir,'tmp2.gff');
    my $result = "$output_folder/agat_sp_keep_longest_isoform_2.gff";
    check_quiet_run(
        "$script --gff " . catfile($input_folder,'agat_sp_keep_longest_isoform_2.gff') . " -o $outtmp"
    );
    check_diff($outtmp, $result, 'output keep_longest_isoform case2');
}
