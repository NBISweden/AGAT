...
my $script = $script_prefix . catfile($bin_dir, "agat_sp_list_short_introns.pl");

{ ok(system("$script -h 1>\/dev\/null") == 0, "help $script"); }

{
    my $dir = setup_tempdir();
    my $outtmp = catfile($dir,'tmp.txt');
    my $result = "$output_folder/agat_sp_list_short_introns_1.txt";
    check_quiet_run(
        "$script --gff " . catfile($input_folder,'1.gff') . " -o $outtmp"
    );
    check_diff($outtmp, $result, 'output agat_sp_list_short_introns.pl');
}
